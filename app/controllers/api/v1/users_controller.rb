class Api::V1::UsersController < ApplicationController
  ...
  def create
    auth_params = SpotifyAdaptor.authorize(params[:code])
    user_data = SpotifyAdaptor.getUserData(auth_params["access_token"])

    user = User.find_or_create_by(user_params(user_data))

    img_url = user_data["images"][0] ? user_data["images"][0]["url"] : nil
    encodedToken = issue_token({token: auth_params["access_token"]})
    user.update(profile_img_url: img_url,access_token: encodedToken)
